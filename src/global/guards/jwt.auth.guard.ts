...
export class JwtAuthGuard implements CanActivate {
    ...
    async getPayload(req: Request, ctx: ExecutionContext) {
        ...
        if (!auth.startsWith("Bearer ")) throw new UnauthorizedException("Invalid toke type is missing Bearer token !")

        try {
            const user: JwtPayload = await this.jwtSubService.verifyToken<JwtPayload>(auth.split(" ")[1], jwtTokenTypeEnum.ACCESS)
            req['user'] = user
            return true
        } catch (error) {
